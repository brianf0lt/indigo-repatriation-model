...
ABRPp = rep.row(c(0,8,4,14,13,15,13,14,13,15,13,14,14,
...
# Manually formatted in an Excel file.

### Table 2
# Tabulate total number released, true population growth (lambda), observed population growth (lambda obs), quasi-extinction probability for 12 repatriation strategies 
...
#### Figure 3
# Use a 3-D surface plot to illustrate extinction probability 
# under a variety of release scenarios with data 
# from the object 'ProbExt'
...
### Figure 5
# Plot extinction probability as a consequence of acclimation effect
# for hatchlings, juveniles, and subadults. This graph is particularly useful
# for the following reason. Managers may be tempted to release hatchlings because
# by releasing them immediately after hatching, they might minimize or
# remove entirely any acclimation effect, because the snake was not kept in captivity
# for an appreciably long time. However, this graph shows that, even if acclimation effect
# is zero for hatchlings, releasing hatchlings still has higher extinction risk
# than juvenile or subadult releases with acclimation effect as high as 60% decrease
# on survival.
...
#### Figure 5, v. 2
#### use a similar four-panel plot as above, but to just plot N with confidence intervals 
#### & without including detection details
par(mfrow=c(2,2), oma=c(5,4,0,0)+0.5, mar=c(0,0,1,1)+0.5, cex.lab=1.7)
scenarios = c("CNFc","CNFp","ABRPc","ABRPp")
labels = c("A","B","C","D")
max = 50	# set max for y axis & panel labels; may have to adjust manually to fit results
for (i in 1:length(scenarios)){
	scenario = scenarios[i]
	plot(1:t, apply(subset(results, Scenario == scenario & Stage == "Breeders")[,-c(1:2)],2,median),
		type="l", lty=1, lwd=4, ylim=c(0,max), axes=FALSE, cex.lab=1.4, cex.axis=2, 
	...
	if(i == 1){	# add legend
		legend(2,0.85*max, c("Median N","95% CI"), inset=0.05, cex=1,
		lty=c(1,3), col=c("black","darkgrey"),
		lwd=c(3,3), box.lwd=2)}
	# manually label extinction probabilities for each scenario; obtain probs from e.g., PECNFc[30]
	if(i == 1){text(6, 0.94*max, expression(italic(P[e])~' = 0.23'), cex=1.2)}
	if(i == 2){text(6, 0.94*max, expression(italic(P[e])~' = 0.09'), cex=1.2)}
	if(i == 3){text(6, 0.94*max, expression(italic(P[e])~' = 0.60'), cex=1.2)}
	if(i == 4){text(6, 0.94*max, expression(italic(P[e])~' = 0.11'), cex=1.2)}
	text(29,0.95*max, labels[i], cex=2)
	title(xlab = "Time (years)", ylab = "Abundance", outer=TRUE, line=3, cex.sub=2)
}